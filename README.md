...
# Instagram job-channel automation

The project now has a separate public-channel mode and an Instagram comment webhook.
The personal resume-matching pipeline remains separate.

## Public channel flow

```text
ATS boards → IT channel filter → banner → Cloudinary → Instagram → save Media ID + apply URL
```

The channel mode includes IT roles at every experience level. It does not use the personal resume score.

### Prepare posts

```powershell
python -m jobhunt channel-prepare --limit 5
```

### Publish prepared posts

Preview first:

```powershell
python -m jobhunt channel-publish --limit 1
```

Publish one real post:

```powershell
python -m jobhunt channel-publish --limit 1 --yes
```

The manifest `out/channel/posts.json` stores the Instagram Media ID and the original official application URL for each published job.

## Instagram comment → apply link

The command below starts the FastAPI webhook:
...
`GET /webhooks/instagram` is the Meta verification endpoint. `POST /webhooks/instagram` receives comment events.

### Any-comment behavior
...
That means any comment on a published job post is eligible. Set `INSTAGRAM_COMMENT_TRIGGER_MODE=keyword` and `INSTAGRAM_COMMENT_KEYWORD=LINK` for keyword-only behavior.

### Reply behavior

Private replies are enabled by default:

```env
INSTAGRAM_PRIVATE_REPLY_ENABLED=true
```

Public replies are optional:

```env
INSTAGRAM_PUBLIC_REPLY_ENABLED=false
```

The private-reply template receives `{url}`, `{company}`, and `{title}` from the matched job record.
...
A duplicate comment/event key is ignored. Comments made by the connected Instagram account itself are ignored to prevent loops.

### ngrok local webhook

Start the webhook locally first:
...
ngrok creates a public HTTPS URL that forwards to the local server. Use:

```text
https://YOUR-NGROK-URL/webhooks/instagram
```

as the Meta callback URL. Official ngrok localhost/webhook guidance: https://ngrok.com/use-cases/share-localhost

### Webhook verification token

Set the same random string in your project and Meta:

```env
INSTAGRAM_WEBHOOK_VERIFY_TOKEN=YOUR_RANDOM_VALUE
```

### Optional webhook signature verification

For production, set your Meta App Secret and enable verification:
...
Keep the real secret values in `.env` only.
